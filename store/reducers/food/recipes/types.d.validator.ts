...
export const isIRecipesState = ajv.compile(IRecipesStateSchema) as ValidateFunction<IRecipesState>;
export default function validate(value: unknown): IRecipesState {
  if (isIRecipesState(value)) {
    return value;
  } else {
    throw new Error(
      ajv.errorsText(isIRecipesState.errors!.filter((e: any) => e.keyword !== 'if'), {dataVar: 'IRecipesState'}) +
      '\n\n' +
      inspect(value),
    );
  }
